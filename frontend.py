...
#this class has been created to hold the methods which deal with 
#taking user inputs, validating them and then returning those values
#so they can be later processed using methods from the backend operations class    
class User_Input_Handling():
    #this method ensures that user inputs are not blank, and if so the user
    #is prompted to re-enter their input to ensure there is a valid string.
    #You can accomplish the same thing using just an if statement after readlines
    #however creating this method significantly reduces code duplication as we are 
    #constantly needing strings from the user.
